...
const atelierHighlights = [
  {
    ...
    body: "Zari, temple borders, and silk textures guided by the rhythm of each event.",
    image: "/images/occasions/bride_side.webp",
    href: "/appointments"
  },
  {
    title: "Design archives",
    body: "Color stories and silhouettes drawn from South Indian bridal rituals.",
    image: "/images/occasions/reception.webp",
    href: "/women"
  },
  {
    title: "Family coordination",
    body: "Bride, groom, kids, and guest looks edited together for the full event calendar.",
...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <section className="atelier-panel py-16 lg:py-24">
        <div className="container-page">
          <div className="grid gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:items-end">
            <div>
              <h2 className="max-w-2xl font-display text-5xl font-semibold leading-tight text-charcoal lg:text-6xl">
                The Amzira atelier, where ceremony begins.
              </h2>
              <p className="mt-5 max-w-xl text-base leading-8 text-charcoal/68">
                From bridal silk to coordinated family looks, every edit is shaped with the quiet discipline of a
                couture studio and the warmth of a South Indian celebration.
              </p>
              <Link className="btn-primary mt-8 gap-2" href="/appointments">
                Plan a bridal session <ArrowRight className="h-4 w-4" aria-hidden="true" />
              </Link>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {atelierHighlights.map((item, index) => (
                <Link
                  key={item.title}
                  href={item.href}
                  className={`atelier-card group focus-ring ${index === 0 ? "md:col-span-2 md:row-span-2" : ""}`}
                >
                  <Image
                    src={item.image}
                    alt=""
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-5 text-white">
                    ...
                    <p className="mt-2 max-w-sm text-sm leading-6 text-white/76">{item.body}</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="mt-12 grid gap-4 lg:grid-cols-3">
            {[
              [Scissors, "Hundreds of quiet handwork hours"],
              [Palette, "Ceremonial color and fabric edits"],
              [BookOpenText, "Archive-led styling guidance"]
            ].map(([Icon, label]) => (
              <div key={String(label)} className="atelier-proof">
      ...
      <section className="ceremony-film py-16 lg:py-24">
        <div className="container-page grid gap-8 lg:grid-cols-[0.78fr_1.22fr] lg:items-center">
          <div>
            <h2 className="max-w-2xl font-display text-5xl font-semibold leading-tight text-white lg:text-6xl">
              A royal wedding procession in motion.
            </h2>
            <p className="mt-5 max-w-xl text-base leading-8 text-white/80">
              A cinematic pause on the homepage: silk, jewelry, procession, and atmosphere before shoppers continue
              into ceremony edits.
            </p>
            <Link className="btn-primary mt-8 bg-gold-dark text-white hover:bg-gold" href="/women">
              Explore ceremony edits
            </Link>
          </div>

          <div className="ceremony-film-frame">
            <video
              className="h-full w-full object-cover"
              src="/images/animations/royal-indian-wedding-process.mp4#t=0.4"
              poster="/images/hero/hero-3.webp"
              autoPlay
              muted
              loop
              playsInline
              preload="metadata"
              aria-label="Royal Indian wedding procession animation"
            />
            <div className="ceremony-film-caption">
              <span>Royal procession</span>
              <span>Muted cinematic loop</span>
            </div>
          </div>
        </div>
      </section>
