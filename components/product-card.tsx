    ...
    <article className="group rounded-md border border-charcoal/10 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-sari">
      <Link href={`/product/${product.slug}`} className="focus-ring block rounded-t-md" aria-label={`View ${product.name}`}>
        <div className="relative aspect-[4/5] overflow-hidden rounded-t-md bg-sandal">
          ...
          <button
            type="button"
            className="focus-ring absolute right-3 top-3 grid min-h-11 min-w-11 place-items-center rounded-full bg-white/90 text-maroon transition hover:bg-maroon hover:text-white"
      ...
      <div className="space-y-3 p-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-gold">{product.categoryName}</p>
        ...
        <div className="flex flex-wrap items-baseline gap-2">
          ...
          {product.basePrice > product.salePrice ? (
            <>
              <span className="text-sm text-charcoal/45 line-through">{formatMoney(product.basePrice)}</span>
