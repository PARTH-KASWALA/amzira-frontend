...
export function CategoryShowcase({ categories, featuredProduct }: { categories: Category[]; featuredProduct?: Product }) {
  ...
  return (
    <div className="collection-showcase grid gap-4 md:grid-cols-2 xl:grid-cols-12">
      <Link
        href={primaryHref}
        className="collection-showcase__primary focus-ring group relative min-h-[460px] overflow-hidden rounded-md bg-charcoal text-white shadow-soft md:col-span-2 xl:col-span-6"
      >
        <Image
          src={primaryImage}
          alt={primaryAlt}
        ...
        <div className="collection-showcase__primary-copy absolute inset-x-0 bottom-0 p-6 sm:p-8">
          <p className="collection-showcase__primary-kicker section-kicker text-gold-pale">Available now</p>
          <h3 className="collection-showcase__primary-title mt-3 max-w-lg font-display text-4xl font-semibold leading-none sm:text-5xl">
            {featuredProduct?.name || activeCategory?.name || "South Indian Kids Lehenga Choli"}
          </h3>
          <span className="collection-showcase__primary-action mt-5 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[0.12em] text-gold-pale">
      ...
      {futureCollections.map((collection) => (
        <Link
          ...
          className="collection-showcase__future focus-ring group relative min-h-[300px] overflow-hidden rounded-md bg-charcoal text-white shadow-soft xl:col-span-2 xl:min-h-[460px]"
        >
          <Image
          ...
          <div className="collection-showcase__future-copy absolute inset-x-0 bottom-0 p-5">
            <p className="collection-showcase__future-kicker text-xs font-semibold uppercase tracking-[0.18em] text-gold-pale">Coming soon</p>
            <h3 className="collection-showcase__future-title mt-2 font-display text-4xl font-semibold leading-none">{collection.name}</h3>
          </div>
        </Link>
      ))}
    </div>
